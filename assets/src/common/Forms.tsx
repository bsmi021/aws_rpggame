...
export class Form extends React.Component<IFormProps, IState> {
  public static Field: React.SFC<IFieldProps> = props => {
    ...
    const handleBlur = (
        e: React.FocusEvent<HTMLInputElement> | React.FocusEvent<HTMLTextAreaElement> | React.FocusEvent<HTMLSelectElement>,
        context: IFormContext
    ) => {
        if (context.validate) {
      ...
      <FormContext.Provider value={context}>
        <form className="form" noValidate={true} onSubmit={this.handleSubmit}>
          ...
          <div className="form-group">
              ...
              <Button type="submit" disabled={false}>Cancel</Button>
          </div>
        </form>
  ...
  private handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      if (this.validateForm()){
          ...
          this.setState({ errors: result.errors || {}, submitted: result.success, submitting: false });
      }
  }
